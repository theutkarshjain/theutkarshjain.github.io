...
$(function () {
    ...
    var scrollBox = $(".full-page > .overlay-container"),
        ...
        demo2Check = $(".bucket-demo2").length,
    ...
    if (demo2Check) {
        scrollBox = $(".bucket-demo2");
    }
    
    /** Smooth Scrolling
     **====================== **/
    $(".bucket-links a,.scrollLink").on('click', function (e) {
        e.preventDefault();
        var hash = this.hash,
    ...
    $('.aboutMe').on('scroll', function () {
        var offsetTop = getOffsetTop($(this), $('.bucket-facts-about-me .bucket-info-list'));
    ...
    $(document,window).on('scroll', function () {
        if (demo2Check || window.matchMedia('(max-width: 767px)').matches) {
            var bodyScrollTop = $(document,window).scrollTop(),
                listoffsetTop = $('.bucket-facts-about-me .bucket-info-list').offset().top - ($(window).height() / 2);
    ...
    $(".bucket-demo2-menu-toggle").on("click", function () {
        $(".bucket-navbar").toggleClass('bucket-demo2-mobile-menu-active');
    });
    /** => End Mobile Menu */
    ...
    $('.bucket-demo2-clients-slider').slick({
        slidesToShow: 6,
        arrows: false,
        dots: false,
        responsive: [
